...
def exec_run(
    ...
    **kwargs: Any,
    ...
    pattern = koxsts.pattern(pgm=pgm, env=env)
    output = koxsts.run(pattern, depth=depth)
    print(koxsts.pretty(output, color=True))
...
def exec_show(
    ...
    **kwargs: Any,
    ...
    koxsts = KOxsts()
    koxsts.show_kcfg(spec_module, claim_id)
...
def exec_view(
    ...
    **kwargs: Any,
    ...
    koxsts = KOxsts()
    koxsts.view_kcfg(spec_module, claim_id)
...
def create_argument_parser() -> ArgumentParser:
    ...
    parser = ArgumentParser(prog='koxsts', description='KOxsts command line tool')
    ...
    run_subparser = command_parser.add_parser('run', help='Run an OXSTS program', parents=[shared_args])
    run_subparser.add_argument('input_file', metavar='INPUT_FILE', type=file_path, help='Path to .oxsts file')
