...
class OxstsSemantics(KCFGSemantics):
...
class OxstsDist:
    ...
    def load() -> OxstsDist:
        return OxstsDist(
            source_dir=OxstsDist._find('source'),
            llvm_dir=OxstsDist._find('llvm'),
            llvm_lib_dir=OxstsDist._find('llvm-lib'),
            haskell_dir=OxstsDist._find('haskell'),
        )
    ...
    def _find(target: str) -> Path:
        """
        Find a `kdist` target:
        * if KOXSTS_<TARGET_IN_SCREAMING_SNAKE_CASE>_DIR is set --- use that
        * otherwise ask `kdist`
        """
        ...
        env_dir = getenv(f"KOXSTS_{target.replace('-', '_').upper()}_DIR")
        if env_dir:
            path = Path(env_dir)
            check_dir_path(path)
        ...
        return kdist.get(f'oxsts-semantics.{target}')
...
class KOxsts:
    ...
    def prove(
        ...
        *,
        ...
        include_dirs = [self.dist.source_dir / 'oxsts-semantics'] + (
        ...
        with self.explore(id=spec_label) as kcfg_explore:
            prover = APRProver(
                ...
                cut_point_rules=['OXSTS.while'],
                terminal_rules=['OXSTS.done'],
            )
            ...
            print(f'  koxsts view {spec_module} {claim_id}')
            print('============================================')
            print('To dump the symbolic execution trace into stdout, run: ')
            print(f'  koxsts show {spec_module} {claim_id}')
            print('============================================')
            if not proof.passed:
                print('To retry the failed/pending proof, run : ')
                print(f'  koxsts prove {spec_file} {spec_module} {claim_id}')
            print('To start the proof from scratch: ')
            print(f'  koxsts prove --reinit {spec_file} {spec_module} {claim_id}')
    ...
    def view_kcfg(
        ...
        proof = APRProof.read_proof_data(proof_dir=self.proof_dir, id=f'{spec_module}.{claim_id}')
        kcfg_viewer = APRProofViewer(proof, self.kprove, node_printer=OxstsNodePrinter(koxsts=self))
        kcfg_viewer.run()
    ...
    def show_kcfg(
        ...
        proof = APRProof.read_proof_data(proof_dir=self.proof_dir, id=f'{spec_module}.{claim_id}')
        proof_show = APRProofShow(self.definition, node_printer=OxstsNodePrinter(koxsts=self))
...
class OxstsNodePrinter(NodePrinter):
    koxsts: KOxsts

    def __init__(self, koxsts: KOxsts):
        from pyk.cterm.show import CTermShow

        super().__init__(cterm_show=CTermShow(koxsts.format))
        self.koxsts = koxsts
    ...
    def print_node(self, kcfg: KCFG, node: KCFG.Node) -> list[str]:
        ...
        res += self.koxsts.format(k_cell).splitlines()
        res += ['env:']
        res += [f'  {line}' for line in self.koxsts.format(env_cell).splitlines()]

        # pretty-print the constraints
        constraints = [ml_pred_to_bool(c) for c in node.cterm.constraints]
        if len(constraints) > 0:
            res += ['constraints:']
            res += [f'  {self.koxsts.format(c)}' for c in constraints]
